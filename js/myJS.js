...
let AboutMe = {
  //create a literal object to ease the adding of another person who worked on the project
  firstName: "Ala",
  lastName: "Schwartz",
  ID: 51651561653,
  image: "<img id='myImg' src='images/cryptonite-crypto.jpg'/>",
  aboutPJ:
    "This is an app which contains all the cryptonite coins, shows the information about them and the live graph of their monetary value.",
};
...
const printCard = (item) => {
  $("#res").append(`<div class="card" >
            <label class="switch">
            <input type="checkbox" class="check1" onclick='addRemove(${JSON.stringify(
              item
            )})'/>
            <span class="slider round"></span>
            </label>
            <div class="card-body"
            <p id="coin_img"><img src=${
              item.image.thumb
            }/>&nbsp<span id="coin_symb"> ${item.symbol.toUpperCase()}</span></p>
            <p class="coin_name">${item.name}</p><br>
            <input type="button"  id="infoButton_${
              item.id
            }" value="More Info" class="btn btn-success btn_info" 
...
const save = () => {
  if (toggleCoins.length <= 5) {
    ...
    alert(
      "The choice is limited to 5 coins only, mark the coin you want to remove"
    );
  }
};
...
const addData = () => {
  toggleCoins.map((item) => {
    ...
    (mySingleData.name = item), dataTo.push(mySingleData);
  });
  console.log(dataTo);
};
